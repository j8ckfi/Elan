...
interface TabsSubtleItemProps extends HTMLAttributes<HTMLButtonElement> {
  ...
  label: string;
  index: number;
  /** Elan extension: closable tab. The selected tab shows ✕ after its label;
   *  unselected (icon-only) tabs close via middle-click or select-then-✕ —
   *  the pinned-tab pattern. (A hover ✕ swap was tried and rejected: it
   *  covers a collapsed tab's entire visible center, so clicking to SELECT
