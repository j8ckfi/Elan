...
// ─── ChatMessage ──────────────────────────────────────────────────────────
// A single transcript entry with a CSS entrance (`.msg-enter`). Pairs with
// InputMessage's onSend: render one per sent/received message. The entrance is
// CSS rather than a framer spring so per-token re-renders during streaming can't
// starve it (that left the streaming reply stuck dim).
  ...
  (
    ...
    return (
      <div
        ...
        className={cn(
          "msg-enter group flex max-w-[80%] flex-col gap-1.5",
